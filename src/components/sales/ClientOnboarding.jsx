...
// const ClientOnboardingModal = () => {
//   const navigate = useNavigate()
//   const handleModalComplete = () => {
//     navigate("/SalesDashboard/corporate-client-onboarding6"); // Replace "/next-page" with the desired route
//   };
...
const ActionButton = ({ label, onClick, bgClass }) => {
  return (
    <button
      onClick={onClick}
      className={`w-[85px] h-[34px] ${bgClass} border border-[#353535] rounded-[4px] text-center font-semibold text-[18px] leading-[21px] text-[#353535] hover:text-[#C61531] hover:border-[#C61531]`}
    >
      {label}
    </button>
  );
...
const Modal = ({ onComplete }) => {
  const [responses, setResponses] = useState({ broker: null, agent: null });
  const [brokerBg, setBrokerBg] = useState({ yes: '', no: '' });
  const [agentBg, setAgentBg] = useState({ yes: '', no: '' });
  ...
  const handleResponse = (field, value) => {
    ...
    // Check if both responses are provided
    if (updatedResponses.broker !== null && updatedResponses.agent !== null) {
      setTimeout(onComplete, 500); // Delay of 1 second before calling onComplete
    }
  };

  const handleClick = (field, value) => {
    handleResponse(field, value);
    if (field === 'broker') {
      setBrokerBg({
        yes: value ? 'bg-red-400' : '',
        no: value ? '' : 'bg-red-500',
      });
    } else if (field === 'agent') {
      setAgentBg({
    ...
    <div className="-mb-[100px] fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center">
      <div className="relative max-w-[800px] h-auto bg-white rounded-[5px] p-8 mx-4">
        ...
        <div className="flex justify-center space-x-4 mb-6">
          <ActionButton
            label="YES"
            onClick={() => handleClick("broker", true)}
            bgClass={brokerBg.yes}
          />
          <ActionButton
            label="NO"
            onClick={() => handleClick("broker", false)}
            bgClass={brokerBg.no}
          />
        </div>
        <div className="w-full text-center font-semibold text-[16px] md:text-[20px] leading-[20px] md:leading-[23px] text-[#353535] mb-6">
        ...
        <div className="flex justify-center space-x-4">
          <ActionButton
            label="YES"
            onClick={() => handleClick("agent", true)}
            bgClass={agentBg.yes}
          />
          <ActionButton
            label="NO"
            onClick={() => handleClick("agent", false)}
            bgClass={agentBg.no}
          />
        </div>
      </div>
